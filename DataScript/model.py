...
def summarize_text(input_text, model, tokenizer):
    input_ids = tokenizer.encode(input_text, return_tensors="pt").to('cpu')  # Перемещаем на CPU
...
def launch_model(file_input, file_output, is_quant_model=True):
    if is_quant_model:
    ...
    make_save_data(file_input, file_output, model, tokenizer)
    return 0
...
if __name__ == '__main__':
    ...
    if allow:
        ...
        model, tokenizer = (GPT2Tokenizer.from_pretrained('RussianNLP/FRED-T5-Summarizer', eos_token='</s>'),
                            T5ForConditionalGeneration.from_pretrained('RussianNLP/FRED-T5-Summarizer').to('cpu')) # Скачивание базы
        print("Исходный размер модели:")
        get_size_of_model(model)

        model = quantize_model(model)  # Квантование модели
